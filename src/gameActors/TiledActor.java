...
// window dimensions
	public static int windowWidth = 208;
	...
	public  TiledActor(String filename, Stage theStage , boolean setworldBounds){	
		...
		tiledCamera = new OrthographicCamera(windowWidth, windowHeight);
	...
	public void draw(Batch batch, float parentAlpha){
		...
		batch.begin();
		// System.out.printf("TiledActor: %.2f x %.2f \n" , tiledCamera.position.x, tiledCamera.position.y);
	}
	...
	public ArrayList<MapObject> getRectangleListContain(String propertyName)
	{
		ArrayList<MapObject> list = new ArrayList<MapObject>();
		for ( MapLayer layer : tiledMap.getLayers() ){
			for ( MapObject obj : layer.getObjects() ){
				...
				if ( props.containsKey("name") && ((String)props.get("name")).contains(propertyName) )
					list.add(obj);
			}
		}
	return list;
